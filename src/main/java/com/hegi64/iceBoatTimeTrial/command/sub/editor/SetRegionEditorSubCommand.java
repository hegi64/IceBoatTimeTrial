...
public class SetRegionEditorSubCommand implements SubCommand {
    private final EditorCommandContext context;
    private final String name;
    private final String description;
    private final RegionType type;

    public SetRegionEditorSubCommand(EditorCommandContext context, String name, String description, RegionType type) {
    ...
    public boolean execute(CommandSender sender, Command command, String label, String[] args) {
        ...
        try {
            RegionBox box = context.regionFromSelection(player, track.getWorld());
            context.trackService().setRegion(track, type, box);
        } catch (IllegalStateException exception) {
            context.send(player, "&c" + exception.getMessage());
            return true;
        ...
        context.send(player, "&aUpdated &f" + type.name() + " &afor &f" + track.getName());
